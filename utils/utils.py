...
def save_checkpoint(state, is_best, checkpoint, itr):  # itr = f'iteration_{str(itr)}
    filename = f'checkpoint_{itr}.pth.tar'
    filepath = os.path.join(checkpoint, filename)
    torch.save(state, filepath)  # 保存模型参数
    # print('successfully saved ' + filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, f'model_best_{itr}.pth.tar'))  # 将名为filepath的文件内容复制到名为...的文件中
...
def create_model(args):
    ...
    elif args.arch == 'cnnres64':
        model = ConvRes([[32, 32], [64, 64]])

    elif args.arch == 'cnndense64':
        model = ConvDense([[32, 32], [64, 64]])
